...
--*/
{
    ...
    //
    if ((Address > 255) || (Address + Size > 255)) {

        //
        // Call the completion handler --- we cannot starve the request
        //
        status = STATUS_END_OF_FILE;
        CompletionHandler( CompletionContext );
        goto AcpiEcOpRegionHandlerExit;
...
--*/
{
    ...
    EcPrint (EC_HANDLER, ("AcpiEcGpeServiceRoutine: Vobj=%Lx, EcData=%Lx\n",
                        GpeVectorObject, ServiceContext));

    AcpiEcServiceDevice (ServiceContext);
