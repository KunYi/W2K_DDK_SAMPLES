...
--*/
{
    ...
    //
    RtlZeroMemory( &inputData, sizeof(ACPI_REGISTER_OPREGION_HANDLER_BUFFER) );
    inputData.Signature     = ACPI_REGISTER_OPREGION_HANDLER_BUFFER_SIGNATURE;
    inputData.AccessType    = ACPI_OPREGION_ACCESS_AS_COOKED;
    inputData.RegionSpace   = ACPI_OPREGION_REGION_SPACE_EC;
    inputData.Handler       = AcpiEcOpRegionHandler;
    inputData.Context       = EcData;

    //
    // Initialize the output
    //
    RtlZeroMemory( &outputData, sizeof(ACPI_UNREGISTER_OPREGION_HANDLER_BUFFER) );

    //
    // Build an irp
    //
    irp = IoBuildDeviceIoControlRequest(
        IOCTL_ACPI_REGISTER_OPREGION_HANDLER,
        EcData->LowerDeviceObject,
        &inputData,
    ...
    if (!irp) {
        ...
        status = STATUS_INSUFFICIENT_RESOURCES;
        goto AcpiEcInstallOpRegionHandlerExit;
    ...
    // Get return value
    //
    EcData->OperationRegionObject = outputData.OperationRegionObject;

AcpiEcInstallOpRegionHandlerExit:
    //
    // done
    //
    return status;
...
--*/
{
    ...
    // Initialize the input
    //
    RtlZeroMemory( &inputData, sizeof(ACPI_UNREGISTER_OPREGION_HANDLER_BUFFER) );
    inputData.Signature = ACPI_UNREGISTER_OPREGION_HANDLER_BUFFER_SIGNATURE;
    inputData.OperationRegionObject = EcData->OperationRegionObject;

    //
    // Build an irp
    //
    irp = IoBuildDeviceIoControlRequest(
        IOCTL_ACPI_UNREGISTER_OPREGION_HANDLER,
        EcData->LowerDeviceObject,
        &inputData,
        sizeof(ACPI_UNREGISTER_OPREGION_HANDLER_BUFFER),
        NULL,
